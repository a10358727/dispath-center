...
from app.approvals import (
    ...
    request_server_delete_approval,
...
from app.hub import request_project_deploy_approval
...
def test_all_fourteen_approval_request_seams_persist_requester_and_actor_audit(
    ...
    running_job_id = db.insert_job(command="sleep 60", status="running")
    approvals.append(
        request_stop_approval(
            db,
            running_job_id,
    ...
    root_candidate = db.upsert_project_candidate(
        server="server-a",
        path="/projects/root",
        name_guess="root-project",
    )
    child_candidate = db.upsert_project_candidate(
        server="server-a",
        path="/projects/root/child",
        name_guess="child-project",
    )
    approvals.append(
        request_import_project_approval(
            db,
    ...
    key_path.write_text("fixture-key-is-never-read\n", encoding="utf-8")
    ...
    assert {approval.kind for approval in approvals} == expected_kinds
    assert len(approvals) == 14
    assert all(approval.requester_actor_id == "requester-actor" for approval in approvals)
    assert all("requester-actor" not in _raw_payload(db, approval.id) for approval in approvals)

    requested = [record for record in read_audit(audit_path) if record["action"] == "approval_requested"]
    assert len(requested) == 14
    assert {record["params"]["kind"] for record in requested} == expected_kinds
    assert all(
        record["actor"]
        == {
            "id": "requester-actor",
