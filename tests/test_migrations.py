...
def test_database_records_version_and_reopen_is_idempotent(tmp_path):
    ...
    assert first._conn.execute("PRAGMA user_version").fetchone()[0] == 18
    first.close()
...
def _revert_to_representative_v15(path) -> None:
    ...
    connection.execute("DELETE FROM schema_migrations WHERE version IN (16, 17, 18)")
...
def _revert_to_representative_v14(path) -> None:
    ...
    connection.execute("DELETE FROM schema_migrations WHERE version IN (15, 16, 17, 18)")
...
def _revert_to_representative_v13(path) -> None:
    ...
    connection.execute("DELETE FROM schema_migrations WHERE version IN (14, 15, 16, 17, 18)")
...
def _revert_to_representative_v12(path) -> None:
    ...
    connection.execute("DELETE FROM schema_migrations WHERE version IN (13, 14, 15, 16, 17, 18)")
...
def _revert_to_representative_v11(path) -> None:
    ...
    connection.execute("DELETE FROM schema_migrations WHERE version IN (12, 13, 14, 15, 16, 17, 18)")
...
def test_representative_v16_upgrade_installs_diverged_trigger_without_backfill(
    ...
    for trigger_name in (
        ...
        assert "instance.state IN ('available', 'diverged')" in sql
    assert upgraded._conn.execute("PRAGMA user_version").fetchone()[0] == 18
    upgraded.close()
...
def test_dispatch_db_cli_upgrade_current_check_backup_and_restore_verify(tmp_path, capsys):
    ...
    assert current["schema_version"] == 18
