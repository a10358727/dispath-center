...
- **Forbidden**:新增其他「直接執行」的 material 寫入端點(唯讀探測如 `test-ssh`、低風險筆記如 `experiment_records`、冪等 hub-sync,以及 server 設定的 add/update/disable(2026-08-26 DG-INFRA-DIRECT-ACTIONS v1 使用者裁定:驗證先行、備份、完整稽核;`server_delete` 仍經核准)是既有的明文例外,新例外需使用者裁定);把 authentication-bookkeeping 例外擴大到 membership、service-account 或 service-token;以 email 查找、合併或識別 actor;first-login-wins 管理員;登入時修改既有 actor 的 material 權限;繞過 `request_*`/`approve()` 直接呼叫執行層。
