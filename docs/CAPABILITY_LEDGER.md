...
| `attempt_driven_ssh` | no | no | no | no | no | no | Current scheduler uses the legacy SSH lifecycle; atomic launch claim and attempt receipts are future WP-2 work. |
...
| `RB-LAUNCH-001` | Legacy SSH dispatch has no durable attempt receipt/atomic launch claim, so a lost launch response cannot distinguish not-started from started. `app/scheduler.py:557` still reverts `running → queued` on any `dispatch_job()` exception, including one raised after `tmux new-session` already forked. | `DG-AMBIGUOUS-LAUNCH-v1` approved 2026-07-27; unchanged in code. Resolved only by WP-2B/2C implementation plus the WP-2D canary. Gate approval is not a fix. |
