...
| `attempt_driven_ssh` | no | no | no | no | no | no | The scheduler still runs the legacy SSH lifecycle. WP-2B added the arbitration primitives it will need — atomic `mkdir` claim, receipt, definite/ambiguous classification, gate §6 resolution — in `app/execution_launch.py`, with a 54-test crash matrix and real-`bash` verification of the launcher bytes. Those are primitives, not this capability: nothing dispatches through them, and `test_scheduler_does_not_import_the_new_launch_module_yet` keeps it that way until WP-2C. |
...
| `RB-LAUNCH-001` | Legacy SSH dispatch still reverts `running → queued` on any `dispatch_job()` exception, including one raised after `tmux new-session` already forked. WP-2B added the arbitration primitives (`app/execution_launch.py`) and the amended `INV-STATE-2`, but deliberately did not wire them into the dispatch path — `test_scheduler_does_not_import_the_new_launch_module_yet` locks that boundary. | WP-2C must route dispatch/reconcile/stop/collect through the attempt path, then the WP-2D canary. Primitives existing is not a fix while the scheduler does not use them. |
