...
|---|---|---|
| `RB-DATASET-001` | **Bounded, not converted.** `DG-DATASET-SNAPSHOT-v1` D-1 ruled that `POST /datasets` stays an approval-free registry declaration — it writes one local row and touches no worker — and is instead labelled. `datasets.reproducible` exists and is `0` for every registry row, including migrated legacy rows. | The reproducible path (`dataset_snapshot`) now exists alongside it. The blocker closes when WP-3B enforces `dataset_not_reproducible` rejection at run-request time; the label alone is not the whole ruling. |
| `RB-SERVER-001` | **Resolved.** All four approved server mutations run the publication protocol (`app/server_publication.py`): `add`/`update` publish a pinned `approved` revision, `disable`/`delete` retire the active one, and every path writes YAML only after a durable journal intent. Compensation follows the observed disk digest (`rolled_back` / continue / `recovery_hold`), and a digest that moved between request and approval fails closed with no write at all. Operator surfaces `GET /server-config/journal` and `POST /server-config/journal/{id}/resolve` are in the authorization catalog; resolution accepts only a digest the journal already records. 16 tests in `tests/test_server_publication.py`. | Targets published this way are eligible for generic claims. `NEW_CLAIMS` still requires the WP-2D canary before enablement. |
