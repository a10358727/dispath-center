...
|---|---|---|---|---|---|---|---|
| `phase0_release_gate` | yes | no | yes | unknown | n/a | no | Local Python 3.10 exact-lock check, compile, static invariant checks, diff check and the current 3,338-test offline suite pass; the CI workflow has not yet produced remote run evidence for this worktree. |
...
| `attempt_driven_ssh` | yes | yes | no | no | no | no | WP-2C runtime dispatch is wired into the scheduler with active-attempt exclusion, immutable revision identity checks, durable prepare/launch intents and D-5 revision-scoped filesystem evidence. A fixed read-only SSH preflight records local/NFS/unknown on the exact active revision; both scheduler selection and the DB claim transaction require `eligible`, while every new revision resets to NULL. SSH fallback remains the default. `DG-WP2D-CANARY-v2` requires an 8-hour canary before the controlled rollout can be proven. |
...
|---|---|---|
| `RB-LAUNCH-001` | **Fixed in code, not yet proven in operation.** The attempt scheduler path now owns dispatch, first outbox intent is atomic, response loss remains `unknown`, and D-5 refuses SSH attempt claims without positive revision-scoped local-filesystem evidence. The legacy branch remains the default while flags are off. | WP-2D v2 canary on a non-production worker (≥20 jobs / 8h, forced response-loss, restart and rollback), then evidence evaluation. |
