...
| `RB-SERVER-001` | **Partially resolved.** Approved `server_add` now runs the publication protocol (`app/server_publication.py`): a pinned approval materializes an `approved`/`active` revision, so the target becomes eligible for generic claims. `tests/test_server_publication.py` asserts the eligibility change, the intent-before-write ordering, and that a failed write compensates from the observed disk digest rather than an assumption. | `server_update`/`server_disable`/`server_delete` still write YAML directly, and no operator recovery surface for `recovery_hold` exists yet. Unpinned legacy approvals stay `legacy_observed` by design. Keep `NEW_CLAIMS=false` until those land. |
