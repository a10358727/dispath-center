...
| `backup_restore` | yes | no | no | unknown | no | no | Operator scripts plus `scripts/restore_drill.py`, which restores a *copy*, runs an integrity check, compares per-table row counts against the live database and measures restore duration. It reports FAIL rather than crashing on a corrupt backup, and flags a restore holding more rows than the source — the one drift direction that means the backup is not of this database. Still no scheduled backup, no drill run against a real production backup, and no RPO/RTO: those need `DG-OPS-SLO`. |
...
| `node_protocol_v2` | yes | yes | no | no | no | no | `DG-NODE-V2-v1` ruled 2026-07-28. WP-4A makes leases server-selected (a v1 agent still sending `job_id` is refused, not ignored), returns held work before selecting new work, adds current-attempt recovery, and converges the canonical Job on a node terminal with the same projection guard the SSH path uses. WP-4B adds staged credential rotation (both secrets valid for a bounded overlap, so a rotation is not an outage) and separates routine retirement — draining, which keeps identity and finishes held work — from revocation, which is immediate and marks nothing failed. `NODE_AGENT_V1_ENABLED` stays off, no node is enrolled, and real-machine activation needs `DG-NODE-CANARY`. |
...
| `immutable_execution_plan` | yes | yes | no | no | no | no | WP-3B/3C derive plans that bind immutable revisions only — pinned commit, published snapshot, specific profile revision, approved target revision, command digest — with a pure preview, a request path that persists an immutable plan plus a pending `plan_run` approval, and approve-time re-verification that rejects a plan whose inputs moved. WP-3C adds promotion: only a ProjectVersion carrying a human-approved `engineering_task_promote` can back a plan, and approve-time re-verification rejects a bundle regenerated since the request. An approved plan now materializes a canonical Job, pinned to the target the plan chose, created at most once per plan. Still not canary-proven and not enabled anywhere. |
