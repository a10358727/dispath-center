...
|---|---|---|---|---|---|---|---|
| `phase0_release_gate` | yes | no | yes | unknown | n/a | no | Local Python 3.10 exact-lock check, compile, static invariant checks, diff check and the current 3,313-test offline suite pass; the CI workflow has not yet produced remote run evidence for this worktree. |
...
| `server_config_management` | yes | no | yes | unknown | unknown | no | All four public add/update/disable/delete request paths now pin exact canonical post-mutation YAML plus before/after digests and materialize through the immutable revision/journal protocol. Publication verifies the written digest before reload/activation; reload failure compensates only when the exact prior bytes are known, otherwise recovery stays fail-closed. Historical unpinned approvals remain honestly legacy. No deployment/canary evidence exists for these changes. |
...
| `execution_attempt_outbox` | yes | yes | no | no | no | no | Attempt claim + first prepare intent are one DB transaction; owner-only delivery covers pinned prepare/launch/stop/collect operations with an effect boundary, while reconciliation remains evidence-driven. Public generic-attempt stop and terminal collect are wired. Node terminal additionally creates an immutable exactly-four completion bundle (`dependency_refresh`, `result_collection`, `notification`, `owner_projection`) with fenced claim, durable outcome and restart recovery. Every rollout flag remains default-off; no production migration/canary evidence exists. |
| `attempt_driven_ssh` | yes | yes | no | no | no | no | WP-2C runtime dispatch is wired into the scheduler with active-attempt exclusion, immutable revision identity checks, and durable prepare/launch intents. SSH fallback remains the default. WP-2D 24-hour canary is still required before enabling flags. |
...
|---|---|---|
| `RB-LAUNCH-001` | **Fixed in code, not yet proven in operation.** The attempt scheduler path now owns dispatch, first outbox intent is atomic, and response loss remains `unknown`; the legacy branch remains the default while flags are off. | WP-2D canary on a non-production worker (≥20 jobs / 24h, forced response-loss, restart), then controlled flag enablement. |
...
| `RB-SERVER-001` | All four approved server mutations publish through the protocol; unpinned historical approvals stay honestly `legacy_observed`. Digest drift, failed writes, reload failure and an unknown disk state each have an explicit outcome, and the operator recovery surface cannot fabricate a revision or edit a digest. Current full offline suite: 3,313 passed. |
