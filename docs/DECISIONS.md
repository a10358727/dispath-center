...
## 補充紀錄：2026-09-03（DG-HARDWARE-EXECUTION v1 P3b：收據、known-good、physical_tools、電源序列）
- 母裁定條款：DG-HARDWARE-EXECUTION v1 H-2、H-4、H-6（含裁定第 5 答的 INV-APPROVAL-1 例外列；本紀錄不改裁定語意）
- 變更：`hardware-receipt-v1`（`app/hardware_receipts.py`：純函式解析 `results/{job_id}/hardware_receipt.json`，封閉 key `device_id`／`device_serial_observed`／`image_sha256`／`tool`／`tool_version`／`verify`／`exit_code`，≤16 KiB；job-finish 在 metrics／映像登記之後對 `hardware_action_v2` 的 Job 存 `hardware_receipts`，並與核准 payload 交叉比對裝置／映像，`missing`＝unknown，永不改任務終態）；known-good 兩條路：(a) `hil_test` 決定 body `mark_known_good=true` 記 `hardware_known_good_intents`，收據 `verify=verified` 且指向已登記映像時才標記（`known_good_source=decision`）；(b) 平台管理員 `POST /api/v2/projects/{id}/hardware-images/{image_id}/known-good` 直接標記（`platform.manage`，`known_good_source=direct`，冪等，僅稽核）；`GET /api/v2/projects/{id}/hardware-receipts`；environment `physical_tools`（封閉字彙 `esptool`／`esptool.py`／`openocd`／`st-flash`／`openFPGALoader`／`vivado`／`uhubctl`，空值不進 digest）＋ H-6 規則：compute／build 模板 argv literal 的 basename 命中宣告工具 → 建卡當下 400 `run_template_invalid`（`physical_tool_in_compute_template`）；argv 新 token `power_sequence`（`power` 模板恰一個）由裝置 `power_control` kind × 序列查封閉表（`usb_relay`：off_on→cycle／reset→reset；`pdu_http`：off_on→cycle／reset→reboot）；`power` 動作的裝置必須宣告 `power_control`（`hardware_device_no_power_control`）；migration 24（`hardware_receipts`、`hardware_known_good_intents`、`hardware_images` known-good 三欄、`environment_revisions.physical_tools_json`）
- 不變：INV-APPROVAL-1 例外表已含 known-good 列（裁定時寫入）；INV-APPROVAL-2／3／4、INV-SSH-6、INV-STATE-* 一字不動；既有 environment／template digest 一個 byte 不變；`is_dangerous()` 黑名單不擴張
