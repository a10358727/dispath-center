...
> Status: 草案，等待使用者具名裁定。裁定前**沒有任何程式碼會被寫入**——
> 這份文件的目的是把「B4 的政策要怎麼定」這個開放問題，收斂成一組可以
> 直接核准或打回的具體參數，模仿本輪 DG-B／DG-C 的裁定模式
> （`docs/DECISIONS.md` 2026-07-19 條目）。
...
- [ ] 核准以上設計為 DG-B4，開始實作（預設 flags 全關，跟 Phase B/A1/D-1
  同樣的「先落地、後啟用」模式）
- [ ] 核准設計但要求修改：___________________________
- [ ] 不做 B4，維持人工建 sync 任務

裁定後請在 `docs/DECISIONS.md` 記一筆具名條目（格式比照 2026-07-19
DG-B／DG-C），我再依裁定內容實作＋補測試。
